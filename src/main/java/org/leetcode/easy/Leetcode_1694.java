...
public class Leetcode_1694 {
    ...
    public static String reformatNumber(String number) {
        ...
        // chList.stream().forEach(System.out::print);

        if (chList.size() < 4) {
            // StringBuilder sb = new StringBuilder();
            // chList.stream().forEach(x -> sb.append(String.valueOf(x)));
            // return sb.toString();
            return chList.stream().map(String::valueOf).collect(Collectors.joining());
        }

        List<String> resultList = new ArrayList<>();
        String str = chList.stream().map(String::valueOf).collect(Collectors.joining());
        ...
        while (leftLength > 0){
            ...
            if(leftLength == 4){
                String subStr = str.substring(start, start + 2);
                resultList.add(subStr);
                resultList.add(str.substring(start + 2, start + 2 + 2));
                break;
            }
        ...
        String ss = resultList.stream().collect(Collectors.joining("-"));
        return ss;
    }
