...
public class Leetcode_1475 {
    ...
    private static int findDiscount(int idx, int[] prices){
        if (idx == prices.length - 1) return 0;
        int tempIdx = idx + 1;
        int discount = 0;
        for (int j = tempIdx; j < prices.length; j++) {
            int tempMin = prices[j];
            if (tempMin <= prices[idx]){
                discount = tempMin;
                break;
            }
        }
    ...
    private static int[] finalPrices(int[] prices) {
        ...
        for (int i = 0; i < prices.length; i++) {
            ...
            // System.out.println("i = " + i + " , discount = " + discount);
            afterDiscount[i] = buyPrice - discount;
        }
    ...
    public static void main(String[] args) {
        ...
        Arrays.stream(finalPrices(prices)).forEach(x -> {
            System.out.printf(x + " , ");
        });
    }
}
